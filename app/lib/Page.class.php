...
class Page
{
	...
	private function first()
	{
		if($this->first)
		{
			...
			$this->page = '<a '.$class.' href="'.$this->url.$this->url_end.'">'.$this->first.'</a>';
		}
		else
		{
	...
	private function prev()
	{
		if($this->prev)
		{
			...
			$this->page .= '<a '.$class.' href="'.$this->url.$prev.$this->url_end.'">'.$this->prev.'</a>';
		}
		else
		{
	...
	private function pages_num()
	{
		...
		if($this->total_num<=5)
		{
			for($index=1;$index <= $this->total_num;$index++)
			{
				...
				{
					$this->page .= '<a href="###" '.$class.' >'.$index.'</span>';
				}
				else
				{
					$this->page .= '<a href="'.$this->url.$index.$this->url_end.'">'.$index.'</a>';
				}
			}
		}
		else
		...
		//总页数大于5页，但当前页没超过3，直接输出，并在最后增加最后页地址
			if($this->page_now<=3)
			{
				...
				$this->page .='<span>...</span>'.'<a href="'.$this->url.$this->total_num.$this->url_end.'">'.$this->total_num.'</a>';
			}
		//当总页数大于5页，且当前页为第4页,
			elseif($this->page_now == 4)
			{
				$this->page .='<a href="'.$this->url.'=1'.$this->url_end.'">1</a>';
				for($index=($this->page_now-2);$index <= 6;$index++)
				{
					...
					{
						$this->page .= '<a href="'.$this->url.$index.'">'.$index.'</a>';
					}
				}
				$this->page .='<span>...</span>'.'<a href="'.$this->url.$index.$this->url_end.'">'.$this->total_num.'</a>';
			}
		//当前页为倒数第三页 不输出最后一页
			elseif( ($this->total_num - $this->page_now) ==  3)
			{
				$this->page .='<a href="'.$this->url.'=1'.$this->url_end.'">1</a>'.'<span>...</span>';
	...
	private function next()
	{
		...
		if($this->next)	
		{
			...
			$this->page .= '<a '.$class.' href="'.$this->url.$next.$this->url_end.'">'.$this->next.'</a>';
		}
		else
		{
	...
	private function last()
	{
		if($this->last)
		{
			...
			$this->page .= '<a '.$class.' href="'.$this->url.$this->total_num.$this->url_end.'">'.$this->last.'</a>';
		}
		else
		{
